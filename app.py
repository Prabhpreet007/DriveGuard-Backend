...
def mouth_aspect_ratio(mouth):
    try:
        # mouth[0] -> 13, mouth[1] -> 14, mouth[2] -> 78, mouth[3] -> 308
...
def process_frame(frame_data, thresholds):
    ...
    thresholds: dict with ear_threshold, mar_threshold, tilt_threshold
    ...
    try:
        if not frame_data:
            return {"error": "No frame data"}, "No frame data"
        ...
        if img is None:
            return {"error": "Invalid image"}, "Image decode failed"

        # speed-up: resize smaller
        img = cv2.resize(img, (320, 240))
        rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

        if face_mesh is None:
            return {"error": "MediaPipe not initialized"}, "FaceMesh not ready"

        results = face_mesh.process(rgb)
        h, w = img.shape[:2]
        now = time.time()

        # reset sound flag every 5 seconds
        if now - alert_state["last_alert_time"] > 5:
            alert_state["sound_played"] = False
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                if ear < thresholds.get("ear_threshold", DEFAULT_THRESHOLDS["ear_threshold"]):
                    alert_state["eye_closed_frames"] += 1
                    if alert_state["eye_closed_frames"] >= 30:
                        alert_detected = True
                        alert_type = "drowsiness"
                ...
                # Yawning (optional, not shown in UI but tracked)
                if mar > thresholds.get("mar_threshold", DEFAULT_THRESHOLDS["mar_threshold"]) and not alert_detected:
                    alert_state["yawn_frames"] += 1
                    if alert_state["yawn_frames"] >= 20:
                        alert_detected = True
                        alert_type = "yawning"
                ...
                # Head Tilt
                if tilt > thresholds.get("tilt_threshold", DEFAULT_THRESHOLDS["tilt_threshold"]) and not alert_detected:
                    alert_state["tilt_frames"] += 1
                    if alert_state["tilt_frames"] >= 30:
                        alert_detected = True
                        alert_type = "tilt"
                ...
                if alert_detected:
                    if (
                        not alert_state["alert_active"]
                        or now - alert_state["last_alert_time"] > 5
                    ):
                        alert_state["alert_active"] = True
                        alert_state["alert_type"] = alert_type
                    ...
                    if (
                        alert_state["alert_active"]
                        and now - alert_state["last_alert_time"] > 2
                    ):
                        alert_state["alert_active"] = False
                        alert_state["alert_type"] = ""
                ...
                result = {
                    ...
                    "status": "ALERT!" if alert_state["alert_active"] else "NORMAL",
                    "sound_alert": alert_state["alert_active"]
                    and not alert_state["sound_played"],
                    "detector_type": "mediapipe",
                }
...
def reset_alerts():
    ...
    return jsonify({"message": "Alerts reset"})
